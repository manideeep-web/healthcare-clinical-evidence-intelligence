...
class LLMClinicalExtractor:
    def __init__(
        self, api_key: str, base_url: str = None, model_name: str = "gpt-4o-mini"
    ):
...
class AuthorizationRiskModel:
    def predict_risk(self, clinical_entities: Dict[str, str]) -> Dict[str, Any]:
        ...
        if history != "none documented" and history != "unknown" and len(history) > 3:
            risk_score += 30
        if (
            "arthroplasty" in procedure
            or "fusion" in procedure
            or "catheterization" in procedure
        ):
            risk_score += 40
        ...
        return {"risk_score": risk_score, "review_tier": tier}
    ...
    try:
        ...
        for page in reader.pages:
            ...
            if extracted:
                text += extracted + "\n"
        return (
            text if text.strip() else "Warning: No extractable text found in this PDF."
        )
    except Exception as e:
        ...
        "entry": [
            {
                "resource": {
                    ...
                    "procedure": [
                        {"procedureString": entities.get("procedure_request")}
                    ],
                    "supportingInfo": [
                        {
                            "category": "Treatment History",
                            "code": entities.get("treatment_history"),
                        }
                    ],
                }
            },
...
def main():
    ...
    st.markdown(
        "Enterprise AI decision support powered by LLM extraction, multi-domain RAG, FHIR interoperability, and audit compliance."
    )
    ...
    provider = st.sidebar.selectbox(
        "Select LLM Provider",
        ["Fallback (Rule-Based)", "OpenAI", "Groq (Fast & Free Tier)"],
    )
    ...
    tab_eval, tab_audit = st.tabs(
        ["📋 Prior Authorization Workbench", "🔒 Enterprise Compliance & Audit Logs"]
    )

    with tab_eval:
        input_method = st.radio(
            "Select Clinical Input Method:",
            ["Manual Text Note", "Upload Patient Medical Record (PDF/TXT)"],
            horizontal=True,
        )
        ...
        if input_method == "Manual Text Note":
            ...
            clinical_note = st.text_area(
                "Paste EHR Provider Note:", value=default_text, height=150
            )
        else:
            uploaded_file = st.file_uploader(
                "Upload Medical Chart (PDF or TXT)", type=["pdf", "txt"]
            )
        ...
        if st.button("Analyze Case"):
            if not clinical_note.strip():
                st.warning(
                    "Please provide or upload a clinical note before running analysis."
                )
            else:
                with st.spinner(
                    "Running AI analysis, policy grounding, and logging compliance trail..."
                ):
                    ...
                    with col1:
                        ...
                        st.json(entities)
                        st.info(
                            f"**Matched Payer Policy (RAG Retrieval):**\n\n{policy}"
                        )
                    ...
                    with col2:
                        ...
                        st.metric(
                            label="Authorization Complexity Score", value=f"{score}/100"
                        )
                    ...
                    st.success(
                        "Analysis Completed & Logged to Compliance Registry Successfully"
                    )

    with tab_audit:
        st.subheader("Session Audit Trail & Compliance Registry")
        st.markdown(
            "Tracks all automated prior authorization evaluations executed during this active session for regulatory reporting."
        )

        if not st.session_state.audit_logs:
            st.info(
                "No cases analyzed yet during this session. Run an analysis in the workbench tab to generate audit records."
            )
        else:
